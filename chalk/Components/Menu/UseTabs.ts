...
import { useControllableState, useId, useSafeLayoutEffect }            from '../../Hooks'
import { callAllHandlers, Dictionary, isUndefined, normalizeEventKey } from '../../Utils'
import { useDescendant, useDescendants }                               from '../../Hooks/use-descendant'
import { useClickable, UseClickableProps }                         from '../../Hooks/use-clickable'
...
export function useTabs(props: UseTabsProps) {
    ...
    * <MenuItem isDisabled>MenuItem 2</MenuItem>
    * <MenuItem>MenuItem 3</MenuItem>
    * ```
    *
    * The manager will only hold references to "MenuItem 1" and "MenuItem 3", since `MenuItem 2` is disabled
    */
    ...
    * ```
    *
    * The manager will only hold references to "MenuItem 1", "MenuItem 2" "MenuItem 3".
    *
...
export interface UseTabOptions {
   ...
   panelId?: string
   /**
    * If `true`, the `MenuItem` won't be toggleable
    */
   isDisabled?: boolean
}
...
export function useTab<P extends UseTabProps>(props: P) {
    ...
    *
    * MenuItem is registered if it is enabled or focusable
    */
 ...
 */
export function useTabPanel(props: Dictionary) {
   const { isSelected, id, children, ...htmlProps } = props
   const { isLazy } = useTabsContext()
   
   return {
      /**
       * Puts the tabpanel in the page `MenuItem` sequence.
       */
      tabIndex: 0,
      ...htmlProps,
...
export function useTabIndicator(): React.CSSProperties {
   ...
   useSafeLayoutEffect(() => {
      ...
      // Horizontal MenuItem: Calculate width and left distance
      if (isHorizontal && tabRect) {
         const { left, width } = tabRect
         setRect({ left, width })
      }
      
      // Vertical MenuItem: Calculate height and top distance
