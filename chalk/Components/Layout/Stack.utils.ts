import { ResponsiveValue, SystemProps } from '../../StyledSystem'
...
export function getStackStyles(options: Options) {
   ...
   const directionStyles: any = {
      column: { marginTop: spacing, marginInlineStart: 0 },
      row: { marginInlineStart: spacing, marginTop: 0 },
   ...
   return {
      ...
      [selector]: mapResponsive(direction, (value: any) => directionStyles[value]),
   }
}
...
export function getDividerStyles(options: Options) {
   ...
   return {
      "&": mapResponsive(direction, (value: any) => dividerStyles[value]),
   }
}
