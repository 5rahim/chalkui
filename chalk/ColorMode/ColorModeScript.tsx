...
import { ColorMode } from "./ColorMode.utils"
...
function setScript(initialValue: Mode) {
   ...
   try {
      persistedPreference = localStorage.getItem("chalk-ui-color-mode") as Mode
   } catch (error) {
      console.log(
         "Chalk UI: localStorage is not available. Color mode persistence might not work as expected",
      )
   }
   ...
   if (colorMode) {
      ...
      root.style.setProperty("--chalk-ui-color-mode", colorMode)
   }
}
